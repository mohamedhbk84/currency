...
bool is_found_internet = true;
class CheckInternetConnection {
  final Connectivity _connectivity = Connectivity();

  /// We assume the initial status is Online
  final controller = BehaviorSubject.seeded(ConnectionStatus.online);
  ...
  Future<void> _checkInternetConnection() async {
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        controller.sink.add(ConnectionStatus.online);
      } else {
        controller.sink.add(ConnectionStatus.offline);
      }
